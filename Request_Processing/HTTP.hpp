...
private:
	...
	static bool parseRequestLine(std::map<std::string, std::string> &map, std::string request);
	...
	std::string		responseMapToString(int statuscode);
	...
	bool				putManager(std::string &put_root, File &file, std::string const &uri, int put_flag = 0, const std::string &response = std::string(""));
...
public:
	...
	static std::map<std::string, std::string> parseMap(std::string &request);
